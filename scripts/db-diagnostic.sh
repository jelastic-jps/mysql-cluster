...
[[ $resp =~ "mysqld is alive" ]] && {
    ...
    SERVICE_STATUS=1;
    writeLog "$MYSQL_STATUS" "ERROR: Service is not running";
    error_found=1;
}
...
resp=$(mysql -u${USER} -p${PASS} -e "show global status like 'wsrep_cluster_status';" | grep 'cluster_status' | awk '{print $2}' 2>&1)
[[ $resp =~ $PRIMARY ]] && {
    ...
    error_found=1;
    writeLog "$GALERA_STATUS" "ERROR: Galera status is - $SECONDARY";
}
