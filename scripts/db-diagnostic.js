...
function execRecovery(scenario, donor, nodeid) {
    ...
    api.marketplace.console.WriteLog("curl --silent https://dot.jelastic.com/download/misc/db-recovery.sh > /tmp/db-recovery.sh && bash /tmp/db-recovery.sh --mysql-user " + user + " --mysql-password " + password + action);
    return cmd({
