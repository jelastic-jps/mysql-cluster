...
JEM=`which jem`
MYSQL=`which mysql`
cmd="CREATE USER '$USER'@'localhost' IDENTIFIED BY '$PASSWORD'; CREATE USER '$USER'@'%' IDENTIFIED BY '$PASSWORD'; GRANT ALL PRIVILEGES ON *.* TO '$USER'@'localhost'; GRANT ALL PRIVILEGES ON *.* TO '$USER'@'%'; FLUSH PRIVILEGES;"
unset resp;
