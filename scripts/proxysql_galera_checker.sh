...
MYSQL_USERNAME=$($PROXYSQL_CMDLINE "SELECT variable_value FROM global_variables WHERE variable_name IN ('mysql-monitor_username')")
MYSQL_PASSWORD=$($PROXYSQL_CMDLINE "SELECT variable_value FROM global_variables WHERE variable_name IN ('mysql-monitor_password')")
#MYSQL_USERNAME=$(echo $MYSQL_CREDENTIALS | awk '{print $1}')
#MYSQL_PASSWORD=$(echo $MYSQL_CREDENTIALS | awk '{print $1}')
