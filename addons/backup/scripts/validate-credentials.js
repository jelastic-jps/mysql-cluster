...
if (service == 's3') {
    cmd = 'rpm -qa | grep -qw s3cmd || yum install -y epel-release && yum install -y s3cmd; s3cmd ls --access_key=${settings.access_key} --secret_key=${settings.secret_key} --host=${settings.s3_host} --no-check-hostname'
