...
galeraSetBootstrap(){
  local node=$1
  local num=$2
  local command="${SSH} ${node} \"[[ -f /var/lib/mysql/grastate.dat ]] && { sed -i 's/safe_to_bootstrap.*/safe_to_bootstrap: ${num}/g' /var/lib/mysql/grastate.dat; }\""  local message="[Node: ${node}] Set safe_to_bootstrap: ${num}"
