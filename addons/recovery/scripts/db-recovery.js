function DBRecovery() {
    ...
    me.execRecovery = function(values) {
        values = values || {};
        log("values->" + values);
        log("curl --silent https://raw.githubusercontent.com/sych74/mysql-cluster/JE-72311-addon/addons/recovery/scripts/db-recovery.sh > /tmp/db-recovery.sh && bash /tmp/db-recovery.sh " + me.formatRecoveryAction(values));
        return nodeManager.cmd({
            command: "curl --silent https://raw.githubusercontent.com/sych74/mysql-cluster/JE-72311-addon/addons/recovery/scripts/db-recovery.sh > /tmp/db-recovery.sh && bash /tmp/db-recovery.sh " + me.formatRecoveryAction(values),
