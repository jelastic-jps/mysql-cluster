...
The topologies *master-slave* or *master-master* are improved with a new horizontal scaling algorithm. The key idea of that is the new cluster member is created via cloning an existing *slave* node. Right after the cloning procedure is completed the database on new cluster member catches up data via binlog replay which definitely will be pretty short in time and guarantees the binlog will not expire unlike the case when the newly added node was created from scratch. Following this improvement we may guarantee cluster fast scaling with no inconsistencies that could arise since the binlog may be overwritten or expire during database cluster lifecycle before cloning. 
